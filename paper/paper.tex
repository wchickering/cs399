...
- GoldSim \right|\right>$) is $\approx 0.1$ (see Fig.  \ref{fig:MSE_SimR}). So
  while $PearSim$ and $CosSim$ are relatively stable after this many common
users, there does remain a significant level of variance.

Our off-line experiment simulates the passing of time, applying ratings to items
in the order they actually occured. The similarity of item pairs is then
estimated each time a user has rated both items such that $N$ is incremented by
one. In this way, we can compare various estimators by comparing their outputs
to each other and to the known $GoldSim$ value for each pair. We proceed like
this for each pair in a test set for $n=1,...,N$ to get varying degrees of
sparsity. We then compute the average squared error of the estimate for each
$n$.
