...
\ref{fig:Histograms}). For example, the $N \geq 80$ dataset we have $\mu_s =
...
The upper panel of Fig. \ref{fig:RatingDist} shows data for the $N \geq 40$
training set while the lower panel shows the same for the $N \geq 80$ training
set. In these density plots, for each rating pair, we randomly choose which is
Rating1 and which is Rating2. The two density plots present similar
distributions. These data, together with the similarity score histograms of Fig.
...
Gaussians. Motivated partly by these density plots and partly by estimator
performance against the training set, we choose $\mu_r = 0.25$ and $\sigma_r =
0.3$ when estimating $TrueSimRand$.

Having chosen values for $\mu_s$, $\sigma_s$, $\mu_r$, and $\sigma_r$ we are now
able to estimate the similarity of item pairs. We do this for each common user
$u$ in the training set in order to determine their average absolute error
$\epsilon_u$. Next, we apply supervised learning techniques to optimize the
value of the parameter $\lambda$ that appears in the definition of the {\em user
predictivity} $p_u$. We do this by minimizing the error in the similarity
estimates made against the training set where each estimate is the weighted
average of the common user's implied prediction. That is, our new estimate for
the similarity of items $A$ and $B$ is
\begin{align}
UserPredSim(A,B) = \frac{\sum_{u \in U_{AB}}{p_u\cdot\hat{y}(r_{u,A}, r_{u,B})}}
{\sum_{u \in U_{AB}}{p_u}},
\end{align}
where, as before, $U_{AB}$ is the set of common users for items $A$ and $B$.
