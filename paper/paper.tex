...
\end{align}
where $\mu_s$ and $\sigma_s^2$ are the average and variance of all similarity
...
$\mathbf{\Sigma_y}$ is the $y$-dependent $2\times2$ covariance matrix. Now,
being that $Y=y$ is the Pearson correlation of the ratings $R_A | y$ and $R_B |
y$ generated by this distribution, if we assume the variance of individual
ratings $\sigma_r^2$ is independent of $y$, we have that
\begin{align}
y = \frac{\mathrm{COV}(R_A | y, R_B | y)}{\sigma_r^2},
\end{align}
where $\mathrm{COV}(R_A | y, R_B | y)$ is the covariance of the ratings $R_A |
y$ and $R_B | y$.  It therefore follows that if the ratings are indeed normally
distributed that the covariance matrix is related to the similarity $y$ by
\begin{align}
...
\begin{align}
\hat{y} &= \argmax_yP(Y=y|\vec{R}=(R_A~~R_B)) 
\\&= \argmax_y\left[\frac{P(\vec{R}=(R_A~~R_B) |
Y=y)P(Y=y)}{P(\vec{R}=(R_A~~R_B))}\right]
\\&= \argmax_y\left[P(\vec{R}=(R_A~~R_B)|Y=y)P(Y=y)\right]
%\\&= 
%\argmax_y\left[\frac{1}{2\pi\sqrt{\left|\mathbf{\Sigma_y}\right|}}\exp{\left(\frac{-(x-y)^2}
%{2\sigma_{2,n}^2}\right)}
%\frac{1}{\sigma_{1}\sqrt{2\pi}}\exp{\left(\frac{-(y-\mu)^2}
%{2\sigma_{1}^2}\right)}\right]
%\\&= \argmin_y\left[\frac{(x-y)^2}{2\sigma_{2,n}^2} +
