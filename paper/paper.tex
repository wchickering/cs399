...
$\sigma_{err,n}^2$ for every value of $n$ separately. To gain a more general 
model, we can model $\sigma_{err,n}^2$ as a function of $n$. Recall that 
$\sigma_{err,n}^2$ is the noisiness of the measurement $Sim$ about $TrueSim$ and $TrueSim = lim_{n \to
...
would decrease as the $sqrt(n)$. Thus we model $\sigma_{err,n}^2$ as:
\begin{align}
...
\begin{align}
ModSim(Sim, n) = \frac{\sigma_{s}^2}{\sigma_{s}^2+\frac{\alpha}{\sqrt{n}}}
...
\begin{align}
P(Y | \vec{R}) = \frac{P(\vec{R} | Y)P(Y)}{P(\vec{R})}.
\end{align}
...
ratings $r_A$ and $r_B$. That is, constraining the parameters $\mu_r$,
$\sigma_r$, $\mu_s$, and $\sigma_s$ to realistic values, this expression defines
a surface that can be to efficiently estimate $y$ given a pair of ratings.
