...
In our first approach, we model true and observed similarity probabilistically. The
goal is to use the observed similarity score directly to approximate the true similarity.
In this way, we can consider an abstact $TrueSim$ which ignores the details of 
...
combination of the mean and the observed similiarity. Since $\sigma_{err,n}^2$ is 
nonnegative, the slope is always less than or equal to one, meaningthat the 
...
This model requires calculating $\sigma_{err,n}^2$ for every value of $n$ 
separately. To gain a more general model, which relates the $\sigma_{err, n}$'s, 
we model $\sigma_{err,n}^2$ as a function of $n$. Recall that $\sigma_{err,n}^2$ 
is the noisiness of the observed $Sim_n$ about $TrueSim$. Since the $TrueSim$ is
the $Sim$ when all users that will rate the items have rated them, $Sim_n$ can be
thought of as sampling $n$ users from the set used to calculate $TrueSim$. 
Although $Sim$ is likely more complicated than an average, the Central Limit 
Theorem motivates the intuition that the variance of $Sim$ would decrease as the 
$\sqrt{n}$. Thus we model $\sigma_{err,n}^2$ as:
\begin{align}
...
The parameters $\mu$, $\sigma_{s}^2$, and $\sigma_{err,n}^2$ can be approximated 
using the method of moments over training data. $\mu$ and $\sigma_{s}$ are
calculated
...
data-specific parameters $\mu$, $\sigma_{s}$ and $\sigma_{err,n}$ to find the best 
...
The second technique is more general. It approximates $\mu$ and $\sigma_{s}$
using the method of moments on the training data of all $TrueSim$ scores. Then,
$\sigma_{err,n}$ is also approximated using a method of moments on the training
data of all $Sim_n$ for each $n$ separately. Here $\sigma_{err,n}$'s are also
considered independently of each other.

The third and most general technique models $\sigma_{err,n}$ as a function of
$n$ using equation 19. As in the previous technique, $\mu$, $\sigma_{s}$ and each
$\sigma_{err,n}$ are approximated using the method of moments over the training
data. Then $\alpha$ is calculated that minimizes the sum of squared errors
between $\sigma_{err,n}^2$ and $\frac{\alpha}{\sqrt{n}}$. This $\alpha$ is used to
...
\end{align}
where $\mu_s$ and $\sigma_s^2$ are the average and variance of all true similarity
scores, respectively. Now, let $\vec{R} = \left(R_A~~R_B\right)$ be a random two
element vector representing the ratings given to items $A$ and $B$.  We model
the conditional distribution of $\vec{R} | Y$ as a multivariate Normal
distribution
\begin{align}
...
Since $y$ is the $TrueSimRand$ score, it can be thought of as the Pearson 
correlation over the distribution of ratings $R_A | Y$ and $R_B | Y$. Assuming
that the variance of individual ratings $\sigma_r^2$ is independent of $y$, we have 
\begin{align}
...
items $A$ and $B$ that most likely produced ratings $R_A$ and $_B$ as
...
$\sigma_r$, $\mu_s$, and $\sigma_s$ to realistic values, the equation
can be used to efficiently estimate $y$ given a pair of ratings.
