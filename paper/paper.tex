...
the two itmes have $n$ common reviewers. Now, let $Y$ be a random variable 
...
true distance. This is reasonable, since prior suggests that the true 
similarity is likely to be near $\mu$.\footnote{It is worth noting that despite
...
To gain a more generality, we model $\sigma_{err,n}^2$ as a function of $n$
...
Since the $TrueSim$ is the $Sim$ when all users that will rate the items 
have rated them, $Sim_n$ can be thought of as sampling $n$ users from the 
set used to calculate $TrueSim$. Although $Sim$ is likely more complicated 
...
large enough that $GoldSim \approx TrueSim$. 

With a measurable $GoldSim$ as our goal, we can apply supervised learning 
techniques. First, we find pairs of items that have at least $N$ common users 
and calculate their $GoldSim$. Next we withold some of the users to produce 
estimated $Sim_n$ for $n=1,...,N$. By doing this over many pairs of items, we 
produce pairs of ($Sim_n$, $TrueSim$) that can be used for training and testing.

Now, the parameters $\mu$, $\sigma_{s}^2$, and $\sigma_{err,n}^2$ can be 
approximated using the method of moments over the training data. The values of 
$\sigma_{err,n}^2$ are fit to the function $\frac{\alpha}{\sqrt{n}}$ and the $\alpha$ 
that minimizes the sum of squared error is computed.

The following graph compares the results of three linear fits, with varying
...
$Sim_n$ and $GoldSim$ are linearly correlated.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{LinearTechniques.png}
	\caption{Comparison of different linear models. For each number of reviewers
    ...
    of guessing the average $GoldSim$ is included.}
    \label{fig:Linear}
\end{figure}

The linear regression technique trades the generality of the model for a better
fit on the training data. As the results show, the trade-off is worth it.
