...
sufficient data. We therefore define $GoldSim = Sim_N$ where $N$ is a number
large enough that $GoldSim \approx TrueSim$. With a measurable $GoldSim$ as our
goal, we can implement a simple predictivity experiment.

To obtain good $GoldSim$ values, we work with subsets consisting of item pairs
with at least either $N=40$ and $80$ common users. These choices are a
compromise between having enough data for statistically significant results and
having a sufficiently large $N$ that $GoldSim \approx TrueSim$. Note that even
with $N=80$ the average absolute error in $PearSim$ (i.e. $\left<\left| PearSim
- GoldSim \right|\right>$) is $\approx 0.1$\footnote{See Fig.
  \ref{fig:MSE_SimR}}.

...

First, we find pairs of items that have at least 
