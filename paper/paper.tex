...
two subsets of this data. The first consists of item pairs having at least 40
common reviewers (i.e. users that have provided a rating for the item). The
second consists of item pairs having at least 80 common reviewers. As discussed
below, we must confine ourselves to item pairs that have many common reviewers
in order to determine a good approximation of the their true similarity.

\subsection*{Experiment}
Ideally, we would know {\em a priori} the true similarity for a set of item
pairs. But both $TrueSimRand$ and $TrueSimPref$ are theoretical constructs and
not metrics we have access to in the real world.  Fortunately, we can achieve
good approximations of either $TrueSim$ given sufficient data. We therefore
define $GoldSim$ = $Sim_N$ where $N$ is a number large enough that $GoldSim
\approx TrueSim$.

With a measurable $GoldSim$ as our goal, we can implement a simple
...
simulates a pair of items at a time when there is sparser rating data. 
We repeat this for $n=1,...,N$ to get varying degrees of sparsity. With the
...
For this experiment, we use Amazon's book data. Books are rated by users on a 1
to 5 star-scale. Before any analysis of the data, we remove the biases from each
rating, as in \cite{Koren2009}:
\begin{align}
\hat{r_{ui}} = r_{ui} - \mu_r - b_i - b_u
\end{align}
where $r_{ui}$ is the raw rating user $u$ gave item $i$, $\mu_r$ is the average
rating over the entire dataset, $b_i$ is the item bias, calculated as the
average rating for item $i$ after $\mu_r$ has been subtracted from each, and 
$b_u$ is the user bias, calculated as the average rating from user $u$ after
$\mu_r$ and $b_i$ have been subtracted from each. 
...
we use $Sim$ = $PearSim$ and $N=70$ for determining $GoldSim$.}
    \label{fig:Linear}
\end{figure}
...
$GoldSim$ as a proxy for $TrueSim$ (See the {\bf The Experiment} section) such
that $\epsilon_u$ is the average absolute error of user $u$'s similarity
predictions.
