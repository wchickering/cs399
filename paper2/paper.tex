...
This assumption is corroborated in the data. The following histogram shows the
number of gold-standard recommendations connected to the top 25 most popular
items, ranked by their value of $\rho$. The histogram also compares the
distribution of predicted recommendations to these items by the
$LatentFeatureRecomender$ with and without popularity taken into account. With
popularity, the predictions follow a similar distribution to the gold-standard
recommendations over these popular items. 
...
Note that without popularity, the recommendations are scattered more uniformly
over the thousands of items in the two catalogues, and so the number of times
these 25 items are recommended is significantly smaller (hardly visible in the
plot). This is because the recommendations are soley based on the similarity in
latent concept space and popular items are not inherently more similar to other
items. This recommender in fact still performs better than the 
$ContentBasedRecommender$, however it fails to exploit this trend in the data.
