...
We now define the $ContentSimilarity$ between items $A$ and $B$ as
\begin{align}
ContentSimilarity(A,B) = \vec{\tau}(A) \cdot \vec{\tau}(B)
\end{align}
where $\vec{\tau}(A)$ is the tf-idf vector representation of item $A$.
...
feature vector. These super-tf-idf vectors act as a proxy for the concept. We
approximate the mapping between two concept spaces by taking inner products of
super-tf-idf vectors. The details of this mapping process are provided in a
subsequent section.
...
%Now for a concept such as {\em formalness}, the words ``sophisticated'' or
%``classy'' may have high value since products with those descriptive words would
%also have a large amount of the {\em formalness} concept in their latent feature
%vectors. 
...
the tf-idf of each word in a product's description by how well that word
...
value of a cell (i, j) in the matrix is the dot product of concept i in the
first space with concept j in the second. The columns and rows of the resulting
matrix represent the relative similarity of the descriptive words between two
concepts.  But the scale of this matrix can be very large. If the inner-product
matrix is used without any normalization as the linear transformation between
the concept spaces, the scale of the transformed products will be inconsistent
with those originally in that space. 
