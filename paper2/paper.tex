...
choose two nodes, one from each of the two sets. Therefore we have $\left(1,750 \times
1,750\right)/2 > 1 \times 10^6$ choices and only $7,000$ of them are correct. Choosing the
correct edges is a formidable challenge to say the least. It is therefore worth
asking: Are some prediction errors better or worse than others?
...
Frequency} (tfidf) from the information retrieval community. In using tfidf, we
...
A key feature of tfidf is that terms appearing in many item descriptions are
discounted via the idf factor. Conversely, rare terms will have larger tfidf
...
this algorithm is quadratic in the number of items, it can be executed relatively
...
tfidf of that word for each product $P$, weighted by the projection of $P$ on
...
sum is over all products $P$ in the catalgoue, tfidf($w$) is the tfidf score
